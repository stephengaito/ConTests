...
local function addCTestTargets(aCodeStream)
  litProgs.setCodeStream('Lakefile', aCodeStream)
  litProgs.markCodeOrigin('Lakefile')
  local lf = {}
  tInsert(lf, "-----------------------------")
  tInsert(lf, "-- ADD targets for all CTests\n")
  tInsert(lf, "local testTargets = {}\n")
  for i, aTestExec in ipairs(build.testTargets) do
    tInsert(lf, "---------------------------------------")
    tInsert(lf, "-- ADD targets for CTests "..aTestExec.."\n")
    tInsert(lf, "tInsert(buildTargets, target(")
    tInsert(lf, "  'build/"..aTestExec..".c',")
    tInsert(lf, "  docFiles,")
    tInsert(lf, "  contextDoc")
    tInsert(lf, "))\n")
    tInsert(lf, "c.program{")
    tInsert(lf, "  'build/"..aTestExec.."',")
    tInsert(lf, "  src={ '"..aTestExec..".c' },")
    tInsert(lf, "  cdir='build',")
    tInsert(lf, "  needs='lua5.2'")
    tInsert(lf, "}\n")
    tInsert(lf, "tInsert(testTargets, target(")
    tInsert(lf, "  'build/"..aTestExec.."-results.lua',")
    tInsert(lf, "  'build/"..aTestExec.."',")
    tInsert(lf, "  './build/"..aTestExec.."'")
    tInsert(lf, "))\n")
  end
  litProgs.setPrepend('Lakefile', aCodeStream, true)
  litProgs.addCode.default('Lakefile', tConcat(lf, '\n'))
  lf = {}
  tInsert(lf, "target('tests', testTargets)")
  litProgs.setPrepend('Lakefile', aCodeStream, false)
  litProgs.addCode.default('Lakefile', tConcat(lf, '\n'))
end
