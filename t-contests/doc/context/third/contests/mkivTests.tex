...
methods which implement both the tracing and mocking. In the next four 
...
particular arguemnts. Finally the last test suite provides an important 
...
define a simple template \type{cmArgUse} which expands into the text 
...
Finally, we provide the Lua \type{createMacro} itself. this method 
...
assert.isEqual(macroBody[2],  "\\def\\test[#1][#2][#3]{%")
assert.isEqual(macroBody[3],  "  \\directlua{%")
assert.isEqual(macroBody[4],  "    thirddata.contests.mockExpansion(")
assert.isEqual(macroBody[5],  "      'test',")
assert.isEqual(macroBody[6],  "      'context',")
assert.isEqual(macroBody[7],  "      {")
assert.isEqual(macroBody[8],  "        '#1',")
