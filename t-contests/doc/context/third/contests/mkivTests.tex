...
methods which implement both the tracing and mocking. In the next three 
test suites we use meta-programming to define either tracing existing 
macros or mocking either \TeX\ or \ConTeXt\ macros. The subsequent test 
suites provide detailed assertions that traced or mocked macros were 
expanded a given number of times with particular arguemnts. Finally the 
last test suite provides an important example of the use of mocked macros 
by mocking the standard \TeX\ \type{\errmessage} macro. 
...
Unlike traced macros, to simulate existing complex macros, some For mocked 
...
\startTestSuite[The trace macros]

In this section we provide the ability to \quote{wrap} an existing macro 
in order to record expansion tracing patterns. We do this by defining the 
\type{\traceTexMacro} and \type{\traceContextMacro} macros. Both macros 
will \type{\let} an existing macro to a different name and then redefine 
...
done, then the original macro with its arguments are expanded as normal. 
We have two distinct tracing macros to deal with the different ways the 
\quote{standard} native \TeX\ macro and a \quote{standard} \ConTeXt\ 
macros delimit their arguments. 

While we could do all of this in native \TeX\ or \ConTeXt, we find it 
easier to make use the Lua based template renderer provided by the 
Literate Programming \ConTeXt\ module. To do this we we define the 
\type{\traceTexMacro} and \type{\traceContextMacro} to make use of 
\type{\directlua} to call the Lua function \type{createTraceMacro}. The 
Lua \type{createTraceMacro} method takes three arguments, the macro name, 
the number of arguments expected by the macro, and the type of macro 
(either \quote{tex} or \quote{context}). 
...
\type{createTraceMacro} expands into the rather more complex \TeX\ macros 
required to wrap the original macro into one that uses the Lua method 
\type{recordExpansion} to make a record of the fact that the original 
macro was expanded with a particular collection of arguments. 

We begin by providing simple Lua templates to deal with the two distinct 
\quote{standard} \TeX\ and \ConTeXt\ argument patterns. In particular, the 
...
with in the two templates \type{ctmTexFormalArgs} and 
\type{ctmContextFormalArgs} respectively. 
...
The \type{ctmFormalArgs} template determines which of the above two 
...
\type{createTraceMacro} before attempting to expand the templates. 
...
define a simple template \type{ctmArgUse} which expands into the text 
...
We can now provide the main template, \type{ctmMain}, which represents the 
actual \TeX\ macros required to provide a tracing wrapper to the particular 
\TeX\ macro to be traced. 
...
Finally, we provide the Lua \type{createTraceMacro} itself. this method 
...
formal arguments require to re-define the wrapped macro. 

The last three sections then build the template environment, find and 
render the \type{ctmMain} template and then split the resulting rendered 
...
\type{createTraceMacro} will be called with \type{aTracingOn} as false. 
...
\startTestCase[createTraceMacro should create the required TeX or ConTeXt macros]
...
assert.length(macroBody, 14)
assert.isEqual(macroBody[1],  "\\let\\oldtest=\\test")
assert.isEqual(macroBody[2],  "\\def\\test[#1][#2][#3]{%")
assert.isEqual(macroBody[3],  "  \\directlua{%")
assert.isEqual(macroBody[4],  "    thirddata.contests.recordExpansion(")
assert.isEqual(macroBody[5],  "      'test',")
assert.isEqual(macroBody[6],  "      'context',")
assert.isEqual(macroBody[7],  "      {")
assert.isEqual(macroBody[8],  "        '#1',")
assert.isEqual(macroBody[9],  "        '#2',")
assert.isEqual(macroBody[10], "        '#3'")
...
assert.isEqual(macroBody[4],  "    thirddata.contests.recordExpansion(")
assert.isEqual(macroBody[5],  "      'test',")
assert.isEqual(macroBody[6],  "      'tex',")
assert.isEqual(macroBody[7],  "      {")
assert.isEqual(macroBody[8],  "        '#1',")
assert.isEqual(macroBody[9],  "        '#2',")
assert.isEqual(macroBody[10], "        '#3'")
...
\def\callTexMockZero#1{%
  \directlua{%
    thirddata.contests.mockExpansion('#1', { }, 'tex')
  }
}
...
\def\callTexMockOne#1#2{%
  \directlua{%
    thirddata.contests.mockExpansion('#1', { '#2' }, 'tex')
  }
}
...
\def\callTexMockTwo#1#2#3{%
  \directlua{%
    thirddata.contests.mockExpansion('#1', { '#2', '#3' }, 'tex')
  }
}
...
\def\callTexMockThree#1#2#3#4{%
  \directlua{%
    thirddata.contests.mockExpansion('#1', { '#2', '#3', '#4' }, 'tex')
  }
}
