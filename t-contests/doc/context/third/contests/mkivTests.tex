...
\startLuaCode
function contests.creaetTraceMacro(macroName, numArgs, argType)
  tex.print(sFmt('\let\old%s=\%s', macroName, macroName))
  local defStr = sFmt('\def\%s', macroName)
  if argType = 'context' then
    for argNum = 1, numArgs, 1 do
      defStr = defStr..'[#'..toStr(argNum)..']'
    end
  else
    for argNum = 1, numArgs, 1 do
      defStr = defStr..'#'..toStr(argNum)
  ...
  tex.print(     defStr..'{%')
  tex.print(    '  \directlua{%')
  tex.print(sFmt('    thirddata.contests.traceMacro("%s", "%s", {',
                  macroName, argType ))
  for argNum = 1, numArgs, 1 do
    tex.print(  '      "#'..toStr(argNum)..'",')
  end
  tex.print(    '    })')
  tex.print(    '  }%')
  tex.print(sFmt('  \\old%s%%'))
  tex.print(    '}')
end
\stopLuaCode
...
\startMocking
  ...
  \assertDefined{startTracingMockCalls}{}
  \assertDefined{stopTracingMockCalls}{}
\stopMocking
\stopConTest
\stopTestCase
\stopTestSuite
