...
We do this over the following collection of test suites. In the first 
three test suites we use meta-programming to define either tracing 
existing macros or mocking either \TeX\ or \ConTeXt\ macros. The 
subsequent test suites provide detailed assertions that traced or mocked 
macros were expanded a given number of times with particular arguemnts. 
Finally the last test suite provides an important example of the use of 
mocked macros by mocking the standard \TeX\ \type{\errmessage} macro. 

\startTestSuite[Lua tracing methods]
...
ways the \quote{standard} native \TeX\ macro and a \quote{standard} 
...
\type{\directlua} to call the Lua function \type{createTraceMacro}. This 
\type{createTraceMacro} takes three arguments, the macro name, the number 
of arguments expected by the macro, and the type of macro (either 
\quote{tex} or \quote{context}).
...
\type{traceMacro} to make a record of the fact that the original macro was 
...
\type{traceMacro} method. Note that all of the original macro's arguments 
are passed into Lua as Lua strings. 
...
assert.isEqual(macroBody[4],  "    thirddata.contests.traceMacro(")
...
\startTestSuite[The mocking environment]
