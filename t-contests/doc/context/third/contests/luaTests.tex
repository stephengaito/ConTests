...
\startTestSuite[ConTestLua environment]
...
\type{ConTestLua} environment. 

The \type{ConTestLua} environment is implemented using the \ConTeXt\ 
buffer environment. and \type{\stopConTestLua} macros use the 
\type{\directlua} macro to call the corresponding \type{something}. 

\startMkIVCode
\appendtoks
  \setuevalue{\e!get\currentbuffer Name}{\currentdefinedbuffer}%
\to \everydefinebuffer

\definebuffer[LuaConTest]

\let\oldStopLuaConTest=\stopLuaConTest
\def\stopLuaConTest{%
  \oldStopLuaConTest%
  \directlua{thirddata.contests.runConTestLuaTest('\getConTestLuaName')}
}
\stopMkIVCode

\startLuaCode
local function runLuaConTestTest(bufferName)
  texio.write_nl(bufferName)
end
\stopLuaCode

\startTestCase[getCurBufferName]
\startConTest
  \assertDefined(startLuaConTest)
  \assertDefined(stopLuaConTest)
  \assertDefined(oldStopLuaConTest)
