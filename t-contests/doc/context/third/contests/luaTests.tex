...
\section[title=Reporting what has happened]

While unit testing is critical, often while developing code the programmer 
needs to understand what has happened. To facilitate this understanding we 
need some tools to report various values to the \ConTeXt\ log. 

We are now able to use the pretty printing code above, to report arbitrary 
values to the \ConTeXt\ log. 
...
Next, we define the \type{addLuaTest} and \type{showLuaTest} Lua methods. 
The \type{addLuaTest} method simply appends the contents of the 
\type{_typing_} buffer to the current case's lua array. The 
\type{showLuaTest} method, makes use of the \type{buildLuaChunk} method to 
log the actual lua chunk used to run the current case's lua tests. The 
\type{buildLuaChunk} method takes either an array of strings or a single 
string containing the Lua code required to run a given Lua test. It then 
inserts it into the text of a Lua method which provides some standard 
local values and returns true. 
...
The \type{runALuaTest} and \type{runCurLuaTestCase} methods together run a 
Lua test case. The \type{runALuaTest} method expects to be provided with 
the text of a Lua test to be run in a given suite and case. The 
\type{runCurLuaTestCase} has the correct patterns of arguments to be a 
test runner. It uses \type{runALuaTest} to actually \emph{run} to contents 
of the current test case's lua table. The \type{runALuaTest} runs both the 
compilation and execution of the test of the Lua test case inside Lua 
\type{pcall}s in order to ensure all errors are caught and properly dealt 
...
\section[title=Additional MkIV assertions]

\startTestSuite[LuaConTest environment]

As literate programmers we are making extensive use of \ConTeXt\ macros as 
interfaces to Lua code. This means that we often want to verify that the 
expansion of a given \ConTeXt\ macro makes associated changes to the state 
of Lua data structures. To do this we would like to run arbitrary Lua 
tests at appropriate points in a ConTest. We can do this by using the 
\type{LuaConTest} environment. 

The \type{LuaConTest} environment is implemented using the \ConTeXt\ 
buffer environment. The \type{\stopLuaConTest} macro uses the 
\type{\directlua} macro to call the \type{runLuaConTest} method with the 
name of the \type{LuaConTest} buffer. We define the 
\type{\getLuaConTestName} macro to be able to capture the correct name of 
the \type{LuaConTest} buffer. 
...
The \type{runLuaConTest} Lua method, takes the \type{bufferName} and uses 
it to get the contents of the buffer, corrects the line endings, and then 
passed the contents to the \type{runALuaTest} method. The results of 
running the Lua test is then provided to the \type{reportMkIVAssertion} 
method to ensure the LuaConTest environment is treated as a ConTest 
assertion. 
...
\startTestCase[should be able to run simple LuaConTests]
\startConTest
  \assertDefined{startLuaConTest}{}
  \assertDefined{stopLuaConTest}{}
  \assertDefined{getLuaConTestName}{}
  \startLuaConTest
    assert.throwsError(assert.fail)
  \stopLuaConTest
  \startLuaConTest
    assert.succeed("this is a test")
  \stopLuaConTest
\stopConTest
\stopTestCase
...
Having defined both the \type{reportLuaAssertion} and the \type{sFmt} 
