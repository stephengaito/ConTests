...
function contests.prettyPrint(anObj, indent)
  local result = ""
  indent = indent or ""
  ...
  elseif type(anObj) == 'table' then
    ...
    for aKey, aValue in pairs(anObj) do
      if type(aKey) ~= 'number' or aKey < 1 or #anObj < aKey then
        table_insert(theKeyValues, { prettyPrint(aKey), aKey, prettyPrint(aValue, indent) })
      end
    end
    table_sort(theKeyValues, compareKeyValues)
...
\startLuaCode
function contests.reportValue(aMessage, aValue)
  texio.write_nl('-----------------------------------------------')
